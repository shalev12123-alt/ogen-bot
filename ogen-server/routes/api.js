  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        new_today: candidates.filter(function(c) {
          return new Date(c.created_at).toDateString() === new Date().toDateString();
        }).length,
        active_jobs: jobs.length,
        leads: leads.length,
        pipeline: {
          new:       candidates.filter(function(c) { return c.status === 'new'; }).length,
          screening: candidates.filter(function(c) { return c.status === 'screening'; }).length,
          phone:     candidates.filter(function(c) { return c.status === 'phone'; }).length,
          interview: candidates.filter(function(c) { return c.status === 'interview'; }).length,
          offer:     candidates.filter(function(c) { return c.status === 'offer'; }).length,
          hired:     candidates.filter(function(c) { return c.status === 'hired'; }).length
        }
      }
    });
  } catch (e) {
